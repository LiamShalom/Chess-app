...
export const bishopMove = (inititalPosition: Position, desiredPosition: Position, team: TeamType, boardState: Piece[]): boolean => {
    //Movement and Attack Logic
    for (let i = 1; i < 8; i++) {
        let multiplierX = (desiredPosition.x > inititalPosition.x) ? 1 : -1
        let multiplierY = (desiredPosition.y > inititalPosition.y) ? 1 : -1
...
export const getPossibleBishopMoves = (bishop: Piece, boardState: Piece[]): Position[] => {
    ...
    for (let i = 1; i < 8; i++) {
        ...
        }else if(tileIsOccupiedByOpponenet(destination, boardState, bishop.team)){
            possibleMoves.push(destination)
            break;
        }else{
            break;
